...
class Base_Config:                          # (Opti for Born 1D)
    ...
    def __init__(self):
        ...
        # Adjust source width
        k = np.sqrt(self.omega / self.D_psi)                         # Dispersion relation (k² = ω/D_ψ)
        lambda_typical = 2*np.pi / k                                 # Wavelength
        self.sigma_emit = lambda_typical / 4                         # Spatial width of the source
...
class Born_2D_Config(Base_Config): # (Opti for Born 2D)
    # --- Space & Time ---
    Nx: int = 200                 # Grid size (x)
    Ny: int = 200                 # Grid size (y), Only use on 2D simulation
    
    x_min: float = -90.0          # Spatial extent (x) (left) 
    x_max: float = 90.0           # Spatial extent (x) (right)
    
    y_min: float = -90.0          # Spatial extent (y) (left)
    y_max: float = 90.0           # Spatial extent (y) (right)
    
    # --- Simulation Control ---
    N_runs: int = 10000            # Number of independent simulated particles
    N_steps: int = 40000          # Steps per particle
    ...
    def __init__(self):
        ...
        # Adjust source width
        k = np.sqrt(self.D_psi / self.omega)                         # Dispersion relation (k² = D_ψ/ω)
        lambda_typical = 2*np.pi / k                                 # Wavelength
        self.sigma_emit = lambda_typical / 4                         # Spatial width of the source