...
@njit(fastmath=True)
def get_drift(psi_val, psi_prev, psi_next, dx, dt, epsilon, alpha):
    """
    ...
    v_max = dx/dt
    if drift > v_max: drift = v_max
    elif drift < -v_max: drift = -v_max
    ...
    for t in range(N_steps):
        ...
        if 1 < idx1 < Nx-2:
            d1 = get_drift(psi_guide[idx1], psi_guide[idx1-1], 
                          psi_guide[idx1+1], dx, dt, epsilon, alpha)
        if 1 < idx2 < Nx-2:
            d2 = get_drift(psi_guide[idx2], psi_guide[idx2-1],
                          psi_guide[idx2+1], dx, dt, epsilon, alpha)
...
def simulate_solo(x_init, N_steps, dt, dx, c, D_psi, omega, gamma,
    ...
    for t in range(N_steps):
        ...
        d = 0.0
        if 1 < idx < Nx-2:
            d = get_drift(psi[idx], psi[idx-1], psi[idx+1], dx, dt, epsilon, alpha)
...
def worker_particle(seed, particle_id, x_space, coupling_code, side, 
    ...
    t1, t2, psi1_acc, psi2_acc, psi_sum_acc, born1_acc, born2_acc, born_sum_acc = simulate_interaction(
        ...
        CFG.emit_amp, CFG.sigma_emit, CFG.alpha, CFG.D_x, CFG.epsilon,
...
def plot_results_page2(data, theory, metrics):
    ...
    # ========================================
    ax4 = fig.add_subplot(gs[1, 1])
    if CFG.coupling_type == "sum":
        rho_2d = theory['rho_2d_fermion']  
    else: 
        rho_2d = theory['rho_2d_boson']
